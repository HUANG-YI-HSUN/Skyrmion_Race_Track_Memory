...
void ForestClassification::initInternal() {
  // cout << "In ForestClassification::initInternal()!!!" << endl ;
...
void ForestClassification::growInternal() {
  // cout << "In ForestClassification::growInternal()!!!" << endl ;
  std::string filename = output_prefix + ".txt" ;
...
void ForestClassification::predictInternal(size_t sample_idx) {
  cout << "In ForestClassification::predictInternal!!!" << endl ;
  if (predict_all || prediction_type == TERMINALNODES) {
    ...
    for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
      ...
      cout << "predictions[0][sample_idx][tree_idx]: " << predictions[0][sample_idx][tree_idx] << endl ;
    }
  } else {
    ...
    cout << "mostFrequentValue(class_count, random_number_generator): " << mostFrequentValue(class_count, random_number_generator) << endl ;
  }
}

void ForestClassification::computePredictionErrorInternal() {
  // cout << "In ForestClassification::computePredictionErrorInternal()!!!" << endl ;
  ...
  for (size_t tree_idx = 0; tree_idx < num_trees; ++tree_idx) {
    for (size_t sample_idx = 0; sample_idx < trees[tree_idx]->getNumSamplesOob(); ++sample_idx) {
      // cout << "trees[tree_idx]->getNumSamplesOob(): " << trees[tree_idx]->getNumSamplesOob() << endl ;
      // cout << "sample_idx: " << sample_idx << endl ;
      size_t sampleID = trees[tree_idx]->getOobSampleIDs()[sample_idx];
      // cout << "tree idx: " << tree_idx+1 << ", trees[tree_idx]->getOobSampleIDs()[sample_idx]: " << sampleID << endl ;
      // cout << "sampleID: " << sampleID << endl ;
      // cout << "getTreePrediction(tree_idx, sample_idx): " << getTreePrediction(tree_idx, sample_idx) << endl ;
      // Read_total_shift+=Level_Tree_Read( getTreePrediction(tree_idx, sample_idx), tree_idx ) ;
      ++class_counts[sampleID][getTreePrediction(tree_idx, sample_idx)];
      // cout << "class_counts[sampleID][getTreePrediction(tree_idx, sample_idx)]: " << class_counts[sampleID][getTreePrediction(tree_idx, sample_idx)] << endl ;
      // cout << "--------------------NEXT------------------------" << endl ;
    }
  }
  ...
  /*for ( int i = 0 ; i < predictions.size() ; i++ ) {
    for ( int j = 0 ; j < predictions[i].size() ; j++ ) {
      for ( int k = 0 ; k < predictions[i][j].size() ; k++ ) 
        cout << predictions[i][j][k] << ", " ;
  ...
  for (size_t i = 0; i < predictions[0][0].size(); ++i) {
    ...
    // cout << "predicted_value: " << predicted_value << endl ;
  ...
  // cout << "This is total reading shift distance: " << Read_total_shift << endl ;
}
...
/* double ForestClassification::getTreePrediction(size_t tree_idx, size_t sample_idx) const {
  ...
  return tree.getPredictionNodeid(sample_idx);
} / -----------------------------------------------------------My Code*/
