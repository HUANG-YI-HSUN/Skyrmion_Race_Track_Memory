...
public:
  ...
  void countSearchTime(std::vector<std::vector<int>> searchList) {
    for ( int j = 0 ; j < searchList.size() ; j++ ) {
      for ( int k = 0 ; k < searchList[j].size() ; k++ ) {
        node_list[searchList[j][k]].usage++ ;
      } // for
    } // for
  } // countSearchTime()
  ...
  void Swap(int limit) {
    for ( int i = node_list.size() - 1 ; i >= 0 ; i-- ) {
      if ( i-1 > -1 && node_list[i-1].node_index >= limit && node_list[i-1].node_index != node_list[i].parent && 
           !node_list[i-1].leaf ) {
        ...
        node_list[i-1] = temp_node ;
                ...
                node_list[i-1].node_index < limit ) )
        break ;
    } // for
  } // Swap()
  ...
  void Frequency_sort(vector<double> &buffer) {
    buffer.clear() ;
