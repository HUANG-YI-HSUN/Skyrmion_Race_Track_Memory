...
void Forest::run(bool verbose, bool compute_oob_error) {
  ...
  if (prediction_mode) {
    ...
    cout << "Finding..." << endl ; // start finding, to make it simpler I use sequential way to simulate parallel way.
...
void Forest::growTreesInThread(uint thread_idx, std::vector<double>* variable_importance) {
  ...
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i, done_tree++) {
      // -------------------------------------------below are my own code------------------------------------------------------
      if ( done_tree > threshold1 ) // dealing with the tree that need to be written in POU manner
        list.insert(i) ;
      ...
      while ( index < node_size ) {
        if ( done_tree > threshold1 ) { // need to write in POU manner
...
void Forest::predictTreesInThread(uint thread_idx, const Data* prediction_data, bool oob_prediction) {
  ...
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i, done_tree++) {
      ...
      if ( list.find(i) == list.end() ) { // dealing with the tree that has to be read in POU manner.
      ...
      else { // dealing with the tree that has to be read in PIU manner.
        for ( int j = 0 ; j < skr.searchList[i].size() ; j++ ) {
          int s_c = 0 ;
          for ( int k = 0 ; k < skr.searchList[i][j].size() ; k++ ) {
