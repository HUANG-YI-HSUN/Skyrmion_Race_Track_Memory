...
void Forest::run(bool verbose, bool compute_oob_error) {
  ...
  if (prediction_mode) {
    ...
    int total_shift_distance = 0, last = 0, ttemp ;
    int backtoroot = 0 ;

    // countSearchTime() ;    // ----------------------------------------For LaRF-----------------------------------------
    /* for ( int i = 0 ; i < num_trees ; i++ ) {
      trees[i]->Frequency_sort(searchList[i], buffer[i]) ;
    ...
    for ( int i = 0 ; i < searchList.size() ; i++ ) {
      for ( int j = 0 ; j < searchList[i].size() ; j++ ) {
        shift_count = 0 ;
        BacktoRootCompress( tree_scope[i], tree_scope_end[i], wordnum_start[i] ) ;
        backtoroot+=shift_count ;
        total_shift_distance+=shift_count ;
        last_apindex = access_port_start[i] ;
        for ( int k = 0 ; k < searchList[i][j].size() ; k++ ) {
          ttemp = Level_Tree_Read( searchList[i][j][k], tree_scope[i], access_port_start[i], wordnum_start[i], tree_scope_end[i], access_port_end[i], wordnum_end[i] ) ;
          total_shift_distance+=ttemp ;
    ...
    cout << "Last: " << last << endl ;
    cout << "Back: " << backtoroot << endl ;
    cout << "Total reading shifting count: " << total_shift_distance << endl ;
...
void Forest::grow() {
  string temp = "" ;
  buffersize = 100000 ; // ( batch_size * tree_size ) / 2
  NLF_Mode = false ;
  ...
  Reset() ;
  Initialize( wordnums, access_port_nums ) ;
  
  // ----------------------above are my own code--------------------------
  // Create thread ranges
  equalSplit(thread_ranges, 0, num_trees - 1, num_threads);
  
  tempbuffer.resize( num_trees ) ;
  buffer.resize( num_trees ) ;
  searchList.resize( num_trees ) ;
  tree_scope.resize( num_trees ) ;
  tree_scope_end.resize( num_trees ) ;
  access_port_start.resize( num_trees ) ;
  access_port_end.resize( num_trees ) ;
  wordnum_start.resize( num_trees ) ;
  wordnum_end.resize( num_trees ) ;

  // cout << thread_ranges.size() << endl ;
  ...
  for ( int j = 0 ; j < buffer.size() ; j++ ) {
    if ( !buffer[j].empty() ) {
      if ( trees[j]->returnSplitValueSize() == buffer[j].size() ) {
        if ( NLF_Mode )
          buffer[j] = Non_Leaf_First_Sort(buffer[j], trees[j]->parent, trees[j]->nodeid) ;
        Level_Tree_Write(buffer[j], j) ;
        buffer[j].clear() ;
      } // if
    } // if
  } // for
  ...
  // cout << "----------------------Print Tree Scope---------------------------" << endl ;
  /* for ( int i = 0 ; i < num_trees ; i++ ) {
    cout << "Tree" << i << "'s information: " << endl ;
    cout << "Start Track: " << tree_scope[i] << ", Start Access Port: " << access_port_start[i] << ", Start Nums: " << wordnum_start[i] << endl ;
    cout << "End Track: " << tree_scope_end[i] << ", End Access Port: " << access_port_end[i] << ", End Nums: " << wordnum_end[i] << endl ;
    cout << endl ;
  } // for */ 
  cout << endl << endl ;
}
...
void Forest::growTreesInThread(uint thread_idx, std::vector<double>* variable_importance) {
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
      ...
      trees[i]->grow(variable_importance);
      int index = 0, node_size = trees[i]->returnSplitValueSize(), track_size = memory[0].size() ;
      while ( index < node_size ) {
        if ( returnBufferSize() < buffersize ) {
          trees[i]->WriteInShowUp(buffer[i], index) ;
...
//-----------------NEW-----------------
  /* int index = 0, node_size = trees[i]->returnSplitValueSize() ;
      while ( index < node_size ) {
        if ( returnBufferSize() < buffersize ) {
          trees[i]->WriteInShowUp(buffer[i], index) ;
          // trees[i]->WriteIn(buffer[i], index) ;       //    NLF and Level order tree
          ...
          wait() ;
          cout << "-------------------Buffer is Full and its contents---------------------" << endl ;
          // printBuffer() ;
          for ( int j = 0 ; j < buffer.size() ; j++ ) {
            if ( !buffer[j].empty() ) {
              if ( tree_scope[j].empty() && trees[j]->returnSplitValueSize() == buffer[j].size() ) {
                Addtrack() ;
                Level_Tree_Write(buffer[j], j, -1, trees[j]->returnSplitValueSize()) ;
                buffer[j].clear() ;
              } // if             
            } // if
          } // for
...
// buffer code ****important****
    /*int index = 0, node_size = trees[i]->returnSplitValueSize() ;
      while ( index < node_size ) {
        if ( returnBufferSize() < buffersize ) {
          trees[i]->WriteIn(buffer[i], index) ;
          trees[i]->WriteIn(tempbuffer[i], index) ;
          index++ ;
        } // if
        else {
          wait() ;
...
void Forest::predictTreesInThread(uint thread_idx, const Data* prediction_data, bool oob_prediction) {
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
      trees[i]->predict(prediction_data, oob_prediction, searchList[i]);
      // Check for user interrupt
#ifdef R_BUILD
      if (aborted) {
        std::unique_lock<std::mutex> lock(mutex);
