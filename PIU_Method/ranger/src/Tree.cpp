...
/*void WriteFile() {
    ...
    float re ;
    int a, b ;
    ofs.open("data5000.data") ;
    if (!ofs.is_open())
      cout << "Error : File can't open." << endl ;

    ofs << "sepal_length,sepal_width,petal_length,petal_width,species" << endl ;
    ...
    for ( int i = 0 ; i < 5000 ; i++ ) {
      for ( int j = 0 ; j < 4 ; j++ ) {
        ...
        b = rand() ;
        b = b % 10 ;
        // cout << (float)((float)b/10) << "b/10" << endl ;
        re = (float)a+((float)b/10) ;
        ofs << re  << "," ;
      } // for
      if ( i < 1600 )
        ofs << "setosa" << endl ;
...
void Tree::grow(std::vector<double>* variable_importance) {
  ...
  while (num_open_nodes > 0) {
    // cout << "i: " << i << endl ;
    node_info anode ; // LaRF and new method
    anode.node_index = i ; // LaRF and new method
    anode.level = depth ; // LaRF and new method
    anode.usage = 0 ; // LaRF and new method
    anode.weight = i+1 ; // LaRF and new method
    // Split node
    //cout << "-------------Start splitNode.-----------" << endl ;
    bool is_terminal_node = splitNode(i);
    anode.node_value = split_values[i] ;
    anode.leaf = is_terminal_node ;
    anode.parent = parent[i] ;
    node_list.push_back(anode) ;
    if ( is_terminal_node )                                      // Swap
      Swap() ;                                                   // Swap
    // Show_up( depth + 1, split_values[i], is_terminal_node, i ) ; // show up method
  ...
  // sort( node_list.begin(), node_list.end() ) ; // show up method

  /* for ( int i = 0 ; i < node_list.size() ; i++ ) {
    cout << node_list[i].node_index << ", " << node_list[i].node_value << ", " << node_list[i].weight << endl ;
  } // for*/
  ...
  /* for ( int i = 0 ; i < nodeid.size() ; i++ )
    cout << nodeid[i] << ", " ;
  cout << endl ; */ 
  // cout << endl << "-----------------------------------Data Size----------------------------------: " << input.size() << endl ;

  // cout << "This is depth: " << depth << endl ;
...
void Tree::predict(const Data* prediction_data, bool oob_prediction, vector<vector<int>> &searchSequence) {
  ...
  searchSequence.resize(num_samples_predict) ; 
  for ( int i = 0 ; i < searchSequence.size() ; i++ )
    searchSequence[i].push_back(0) ;
    ...
    while (1) {
      ...
      if (prediction_data->isOrderedVariable(split_varID)) {
        if (value <= split_values[nodeID]) {
          ...
          // cout << "Move Left and nodeId: " << nodeID << endl ;
          searchSequence[i].push_back(nodeID) ;
        } else {
          // Move to right child
          nodeID = child_nodeIDs[1][nodeID];
          // cout << "Move Right and nodeId: " << nodeID << endl ;
          searchSequence[i].push_back(nodeID) ;
        }
      } else {
...
void Tree::createPossibleSplitVarSubset(std::vector<size_t>& result) {
  ...
  // cout << num_vars << "data->getNumcols" << endl ;
...
bool Tree::splitNode(size_t nodeID) {
  ...
  // cout << "The node id in splitNode: " << nodeID << endl ;
  // cout << "The possible split varIDs size: " << possible_split_varIDs.size() << endl ; 
  ...
  size_t split_varID = split_varIDs[nodeID];
  // cout << "Split_varID: " << split_varID << endl ;
  ...
  // cout << "Starting position: " << start_pos[left_child_nodeID] << endl ;
  ...
  parent.push_back(nodeID) ;
  // child.push_back(split_value) ;
  createEmptyNode();
  start_pos[right_child_nodeID] = end_pos[nodeID];
  // cout << "Ending postion: " << start_pos[right_child_nodeID] << endl ;
  float dataget ;
 // for ( size_t i = 0 ; i < sampleIDs.size() ; i++ )
       // cout << sampleIDs[i] << endl ;
  ...
  if (data->isOrderedVariable(split_varID)) {
    ...
    while (pos < start_pos[right_child_nodeID]) {
      // for ( int i = 0 ; i < 8 ; i++ ) {
       // cout << data->get_x(sampleIDs[i], split_varID ) << endl ;
      // } // for 
      size_t sampleID = sampleIDs[pos];
      // cout << "Now position: " << pos << endl ;
      dataget = data->get_x(sampleID, split_varID) ;
      // cout << "Data value: " << dataget << endl ;
      // cout << data->get_x(sampleID, split_varID) << endl ;
      if (dataget <= split_value) {
        // cout << "Go to left." << endl ;
        // If going to left, do nothing
        ++pos;
      } else {
        // cout << "Go to right and "  ;
        // If going to right, move to right end
        --start_pos[right_child_nodeID];
        // cout << "the ending position: " << start_pos[right_child_nodeID] << endl ;
